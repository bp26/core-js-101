...
function timeSpanToString(startDate, endDate) {
  ...
  const timeTemplate = (time) => `${(`${time}`).padStart(2, '0')}`;


  return `${timeTemplate(hours)}:${timeTemplate(minutes)}:${timeTemplate(seconds)}.${(`${timespan}`).padStart(3, '0')}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const angle = Math.abs((hours + minutes / 60) * 30 - minutes * 6);
